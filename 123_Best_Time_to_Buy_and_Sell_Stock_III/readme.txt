...
网上查了查，想了想，其实可以一开始遍历数组一遍，得到left和right数组，left[i]表示从[0,i]区间进行操作能获得的最大收益。right[i]表示从[i, prices.size()-1]区间进行操作能获得的最大收益，求left数组和right数组的时间复杂度都是O(n)， 因此总的时间(加上划分)复杂度就是O(n)
