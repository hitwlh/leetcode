...
记号：
数组A，B： A[0, 1, ..., m-1] B[0, 1, ..., n-1]，假设把A和Bmerge到一起的有序数组是C
下面就看看如何实现找C中的第k大。
...
只要A和B都不空，就一直做下面的步骤：i取A有效部分的中位数，j取B有效部分的中位数
    ...
    (1) A[i] < B[j]
    在C里，B[j]前至少有i+j+1个数，也就是j' >= i+j+2
    在C里，A[i]前至多有i+j个数，也就是i' <= i+j+1
    若k > i + j + 1, 可以把A[0, ..., i]排除
    若k <= i + j + 1，可以把B[j, ..., n-1]排除

    (2) A[i] > B[j]
    在C里，B[j]前至多有i+j个数，也就是j' <= i+j+1
    在C里，A[i]前至少有i+j+1个数，也就是i' >= i+j+2
    若k > i + j + 1, 可以把B[0, ..., j]排除
    若k <= i + j + 1，可以把A[i, ..., m-1]排除

    (3) A[i] = B[j]
    i' = i+j+1
    j' == i+j+2
    若 k = i+j+1 或 k = i+j+2 直接返回A[i]
    若 k < i+j+1 排除 A[i, ..., m-1] B[j, ..., n-1]
    若 k > i+j+2 排除 A[0, ..., i] B[0, ..., j]
